...
* \fn compute_weight(double distance_i, const double h)
* \brief Compute the Euclidian distance between two points.
*
* \param a : Point a.
* \param b : Point b.
* \return Returns the Euclidian distance between a and b.

double compute_weight(double distance_i, const double h)
{
	return std::exp(-2.0 * std::pow(distance_i, 2.0) / (h*h)); // / sum_distances ;
}
*/
...
{
	double alpha = 1.0; //Luminance
	double beta = 1.0; //Contrast
	double gamma = 0.5; //Structure
	double const_k = 1.0;
	...
	for (int i = 0; i < regptset.npts(); i++)//loop on each point of registered
	{
		...
		const size_t nMatches_Reg = m_kdtree2.radiusSearch(&query_pt[0], std::pow(search_radius_neighborhood,2.0), ret_matches_Reg, params); //not Euclidean space => pow(radius,2)
		...
		for (size_t cpt_reg = 0; cpt_reg < nMatches_Reg; cpt_reg++)
		{
			...
			ret_distance_Reg.push_back(std::sqrt(ret_matches_Reg[cpt_reg].second)); //not Euclidean space => sqrt(distance) 
...
int main(int argc, char** argv) {
	...
	Note that you need to execute the code in both forward and backward (-i) and do your own Mean on the PC_MSDM field from the output files.
		Windows : 
			Forward  =>	perceptualmetric.exe cube.xyz cube_Octree_perc0_3.xyz pc_msdm  -fq -r 0.007 -knn 5 -a 2
			Backward =>	perceptualmetric.exe cube.xyz cube_Octree_perc0_3.xyz pc_msdm  -fq -r 0.007 -knn 5 -a 2 -i
		Linux : 
			Forward  =>	./perceptualmetric     cube.xyz cube_Octree_perc0_3.xyz pc_msdm  -fq -r 0.007 -knn 5 -a 2
			Backward =>	./perceptualmetric     cube.xyz cube_Octree_perc0_3.xyz pc_msdm  -fq -r 0.007 -knn 5 -a 2 -i
